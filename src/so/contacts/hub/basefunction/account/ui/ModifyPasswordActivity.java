...
public class ModifyPasswordActivity extends BaseActivity implements OnClickListener, IAccCallback
{
        ...
        if (NetUtil.isNetworkAvailable(this))
        {
            if (isMoblieRight())
            {
                ...
                AccountManager.getInstance().sendCaptchar(this, mobile, actionCode, new IAccCallbackAdv<String>()
                {
                    ...
                    {
                        Toast.makeText(ModifyPasswordActivity.this, R.string.putao_send_captcha_error,
    ...
    private void verifyCaptcha()
    {
        if (NetUtil.isNetworkAvailable(this))
        {
            if (isMoblieRight())
            {
                String accName = mUsernameET.getText().toString().trim();
                accName = accName.replace(" ", "");
                captchar = mPasswordET.getText().toString().trim();
                AccountManager.getInstance().verifyCaptchar(this, "200005", accName, captchar, this);
            }
            else
            {
                Toast.makeText(this, R.string.putao_charge_phonenum_error, Toast.LENGTH_SHORT).show();
        ...
        public void onReceive(Context context, Intent intent)
        {
            ...
            if (!TextUtils.isEmpty(action) && action.equals(SMS_RECEIVED_ACTION))
            {
                ...
                for (Object pdu : pdus)
                {
                    ...
                    if (!TextUtils.isEmpty(sender) && sender.equals(phoneNum) || !TextUtils.isEmpty(content)
    ...
    {
        //验证码验证成功之后，跳到密码修改页
        Intent intent = new Intent(ModifyPasswordActivity.this, SetPasswordActivity.class);
